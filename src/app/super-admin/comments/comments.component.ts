...
export class CommentsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.socket.on('connect', () => {
      ...
      var dda = this.dataService.getData('userData');// x8WIv7-mJelg7on_ALbx
  ...
  async sendMessage(msg: string, check: boolean, item) {
    if (this.imageChangedEvent && this.uploadForm.status == 'VALID' &&
      check == false) {
  ...
  onUserSelect(e) {
    ...
    console.log("adddddddddd",this.reciepent_id_list)
  }
  ...
  getMessages() {
    return Observable.create((observer) => {
      this.socket.on('document-comments', (message) => {
        ...
        //       for (let j = 0; j < this.chatList[i].replies.length;j++){
        //         if(this.chatList[i].replies?.length > 0 && this.chatList[i].replies != undefined || null){
        //           let p = this.chatList[i].replies[j].audio
        //           let q = p?.split('.')
        //           this.chatList[i].replies[j].filterchartRe = q[1];
        //         }
  ...
  backClicked() {
    this._location.back();
    this.dataService.setData("commentsToProjects", "commentsToProjects")

    let obj = this.dataService.getData("commentsVisit");
    if (obj.hasOwnProperty('category_id')) {
      this.dataService.setData("revisit", obj)
    }
    else {
